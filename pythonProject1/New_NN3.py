...
from PyQt5.QtWidgets import QLabel, QGridLayout, QTableWidget
...
import copy
import numpy as np
from dir_for_func.word_doc import  create_word_files
...
class ExcelTableWidget2(QWidget):
    ...
    def __init__(self, parent, i, FIO):
        ...
        #кнопка
        self.baton4 = QPushButton(self)
        self.baton4.setText('Формирование Word документов')
        self.baton4.clicked.connect(self.create_word_files)
        self.layout_h2.addWidget(self.baton4)
        #надпись
...
#C:\Users\anikitin\PycharmProjects\pythonProject1\addition
    ...
    def create_word_files(self):
        dfs = pd.read_excel('addition/конкурс.xlsx', sheet_name=None)
        try:
            QMessageBox.information(self, 'Уведомление', 'Протоколы успешно созданы')
            create_word_files(self, dfs)
        except Exception as e:
    ...
    def get_dep_and_row(self, wd, blank1, df1, FIO1, curenttext1,dep,counter_row =[]): # функция достающая количество отделов и возвращаюшая их
        # Для программиста :  сократить код , а то тут большая часть действий просто не нужна
        ws=wd
        blank=blank1
        df=df1
    ...
    def generate_table_from_excel_using_func(self, ws, blank, df, FIO, curenttext, dep, wb,path,sheet_list):  # функция заполняющая таблицы данными по отделам
